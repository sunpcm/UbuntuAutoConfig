...
- Deprecated `ubuntu-server/ansible` and `wsl-dev/ansible` paths must stay empty; legacy compat entrypoints must not read archived vars.

## Legacy / archived code — do not use as a source

- `wsl-dev/bootstrap.sh`, `ubuntu-server/bootstrap.sh`, root `playbook.yml`, root `setup_wsl.yml` are **compatibility shims only** — they forward to the unified implementation. Don't extend them or cite them in new docs/automation.
- `archive/legacy-implementations/` is historical reference only; it is not executable and must not be a config source.
